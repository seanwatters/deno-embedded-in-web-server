...
thread_local! {
    static MAIN_WORKER: RefCell<MainWorker> = {
        ...
        RefCell::new(MainWorker::bootstrap_from_options(
            main_module,
            permissions,
            options,
        ))
...
fn run_js(src: &str) -> DetachedBuffer {
    MAIN_WORKER.with(|main_worker| {
        let mut main_worker = main_worker.borrow_mut();        
